  ...
  describe("Rent Process", () => {
    it("Should create an auction", async () => {
      for await (let currentRent of RentsData) {
        ...
        const tx = await (
          await rent.createRent(nft.address, id, price, [feeReceiver.address])
        ).wait();
    ...
    it("Should set correctly the price in USD", async () => {
      for await (let currentRent of RentsData) {
        ...
        expect(
          Math.round(
            ((await rent.getRatePrice(RentId, Rent.tokens[0]))
              .div(10 ** 8)
              .toNumber() /
              10 ** 12) *
              84679030,
    ...
    it("Should get batch Rents", async () => {
      ...
      allRents.forEach((Rent: any, i: any) => {
        ...
        expect(Rent.amount).to.be.equal(1);
        expect(Rent.price).to.be.equal(price);
        expect(Rent.tokens[0]).to.be.equal(feeReceiver.address);
        expect(Rent.duration).to.be.equal(duration);
        expect(Rent.status).to.be.equal(Rent_STATUS.created);
    ...
    it("Should not cancel Rents inRent", async () => {
      ...
      const daysOfRent = 2;
      const cost = (await rent.getRatePrice(Rents[3], feeReceiver.address)).mul(
        daysOfRent,
      );
      ...
      await rent
        .connect(accounts[2])
        .rent(Rents[3], daysOfRent, feeReceiver.address, {
          value: cost,
        });
    ...
    it("Should not buy cancelled Rents", async () => {
      await expect(
        rent.rent(Rents[0], 3600, feeReceiver.address),
      ).to.be.revertedWith("");
    });

    it("Should rent by calculated amounts", async () => {
      const daysOfRent = 2;
      const cost = (await rent.getRatePrice(Rents[1], feeReceiver.address)).mul(
        daysOfRent,
      );
      ...
      const receipt = await (
        await rent
          .connect(buyer)
          .rent(Rents[1], daysOfRent, feeReceiver.address, { value: cost })
      ).wait();
    ...
    it("Should no rent while the nft is already rented", async () => {
      ...
      await rent.rent(rentId, timeRent.duration, feeReceiver.address, {
        value: price,
      });
      const logs = getLogs(rent.interface, tx);
      const RentId = logs.find(
      ...
      Rents.push(RentId);
      await expect(
        rent.rent(rentId, timeRent.duration, feeReceiver.address, {
          value: price,
        }),
    ...
    it("Should redeem after time rent is passed", async () => {
      ...
      await rent
        ...
        .rent(rentId, timeRent.duration, feeReceiver.address, {
          value: (
            await rent.getRatePrice(rentId, feeReceiver.address)
          ).mul(timeRent.duration),
        });
      const logs = getLogs(rent.interface, tx);
      const RentId = logs.find(
        ({ name }: { name: string }) => name === "RentAuctionCreated",
