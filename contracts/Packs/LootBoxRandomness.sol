...
library LootBoxRandomness {
    ...
    function setOptionSettings(
        ...
        require(
            _typeIds.length == _typeInferiorLimit.length,
            "params lenght mismatch"
        );
        require(
            _typeIds.length == _typeSuperiorLimit.length,
            "params lenght mismatch"
        );
    ...
    function _mint(
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            uint256[] memory typesMinted = new uint256[](
                settings.typeIds.length
            );
            ...
            for (; totalMinted < settings.mintLimit; ) {
                uint256 typeIndex = uint256(
                    _random(_state) % settings.typeIds.length
                );

                if (
                    typesMinted[typeIndex] >=
                    settings.typeSuperiorLimit[typeIndex]
                ) continue;
                _sendTokensWithType(
                    _state,
                    _toAddress,
                    settings.typeIds[typeIndex],
                    1
                );
    ...
    function _sendTokensWithType(
        ...
        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenIndex = uint256(
                _random(_state) % _state.typeToTokens[_typeId].length
            );
