...
async function main(): Promise<void> {
  ...
  const fileData = loadJsonFile(`addresses/addresses.${network.name}.json`);
  ...
  const draculHolders = await getHoldersAmounts(
    dracul,
    erc1155Tokens[0].balances,
    1,
  );
  console.log("draculHolders");
  const endersDraculHolders = await getHoldersAmounts(
    endersGate,
    erc1155Tokens[1].balances,
    215,
  );
  console.log("endersDraculHolders ");
  const endersErossHolders = await getHoldersAmounts(
    endersGate,
    erc1155Tokens[2].balances,
    230,
  );
  console.log("endersErossHolders ");
  const erossHolders = await getHoldersAmounts(
    eross,
    erc1155Tokens[3].balances,
    1,
  );
