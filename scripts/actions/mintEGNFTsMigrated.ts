...
import { EndersGate__factory } from "../../types";
...
async function main() {
  ...
  const draculFile = `nfts_holders/holdersDraculInHarmony.json`;
  const draculData = loadJsonFile(draculFile);
  const erossFile = `nfts_holders/holdersErossInHarmony.json`;
  const erossData = loadJsonFile(erossFile);
  const addressesData = [
  ...
  for (let i = 0; i < addressesData.length; i++) {
    ...
    const NFTsToMint = [];
    if (draculData.result[i] > 0) {
      NFTsToMint.push({ balance: draculData.result[i], id: 215 });
    }
    if (erossData.result[i] > 0) {
      NFTsToMint.push({ balance: erossData.result[i], id: 230 });
    }
    ...
    console.log("Transfered address:", address);
  }
}
